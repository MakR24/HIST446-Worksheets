...
> I typed the command paste, then wrote one string, the "Hello". Then I ended that string, wrote out a comma, then I added the second string, "Ball State Students", then closed the parantheses. This caused the two strings to combine into one string of words. 
...
>Each line is some kind of equation, the thing decides whether or not the equation (?),  is correct or not, and depending on the answer it will come back as true or false
...
1875 < 1970 & 1875 <1980
```
No, I literally do not know what to do it doesn't make any sense. Thanks!
---
...
```{r}
VeryDescriptiveName <- 2
EvenMoreDescribeWow <- 5
VeryDescriptiveName * EvenMoreDescribeWow
```
...
> It took the list of numbers and multiplied each one by two. 
...
> It's the same thing, but since you specified the 3, it only multiplied the 2 by the third variable

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
ReallyMyValue <- c("cat", "dog", "fox", "rat", "frog")
```
...
> It took me to the R rounding of number thing again, which is correct because that has the segment about what the floor does. 
...
> Z is just the answer to the equation that was put in the previous line of code. 
...
The documentation was able to tell me that the dataset contains a list of address in an address book from 1965-1985
> 
...
> I had to do the ? thing where you can get the Help menu to pop up. 
...
>This is a dataset containing the information of different judge position, and who was in them, when they were appointed, fired, or died. 
