...
> Because of the negative sign in front of the C, instead of cherry picking out each number in order that you wanted from the set, it takes away those specific numbers that you take out, excludes them from the set. 
...
>So to assign a criteria to get your "True" variables in the set, you have to create an equation like we learned how to do last week, and make sure to assign it to x. 
...
This line of code assigned a letter value to each of the numbers in the list. 
...
```{r}
matrix(c(28, 32, 36, 40, 44), byrow = TRUE, nrow = 1)
```

(@) Can you create a matrix that has 3 rows with only even numbers from 2 to 18? 
```{r}
matrix(c(2, 4, 6, 8, 10, 12, 14, 16, 18), nrow = 3)
```
...
> We get a lot more rows that are defined with scepfic descriptors because it is connected to a bunch of data and not just  numbers that we put in ourselves

(@) Can you find just the city and state for the first 5 entries? 
```{r}
 head(gayguides$city)
head(gayguides$state)
```
...
almshouse_admissions[3:8]

```

> I was really interested in learning the dates between admittance and discharge. I can't figure out a way to just get just those two collums. 
...
>This code picks out a list of the cities in the dataset, specifically 100-110 and lsits them all out
